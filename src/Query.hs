...
import Data.Char (chr, isPrint, isAscii, isLower)
import Data.Either (partitionEithers)
import Data.Void (Void)
import Data.Word (Word8)
import Text.Megaparsec ( (<|>), some, many, Parsec, MonadParsec(takeWhile1P, takeWhileP) )
...
searchQueryParser = do
  symbol "search"
  params <- some $ lexeme (Left <$> letterStringParser <|> Right <$> tagParser)
...
word = takeWhile1P Nothing (( \x -> isAscii x && isLower x || x == '-' ) . toChar)
